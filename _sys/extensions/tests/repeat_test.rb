...
class RepeatingTaskTest < Minitest::Test
  def setup
    ...
    @strict_daily_repeating_file = File.join(@root_dir, "#{@today.strftime('%Y%m%d')}.mytask.@8d.txt")

    @weekly_repeating_file = File.join(@archived_dir, "#{(@today - 14).strftime('%Y%m%d')}.mytask.2w.txt")
    @strict_weekly_repeating_file = File.join(@root_dir, "#{@today.strftime('%Y%m%d')}.mytask.@5w.txt")

    @monthly_repeating_file = File.join(@archived_dir, "#{(@today << 3).strftime('%Y%m%d')}.mytask.3m.txt")
    @strict_monthly_repeating_file = File.join(@root_dir, "#{@today.strftime('%Y%m%d')}.mytask.@6m.txt")

    @weekday_repeating_file = File.join(@archived_dir, "#{(@today - (@today.wday - 1)).strftime('%Y%m%d')}.mytask.monday.txt")
    @strict_weekday_repeating_file = File.join(@root_dir, "#{@today.strftime('%Y%m%d')}.mytask.@friday.txt")
  ...
  def test_daily_repeating_task
    ...
    expected_daily_file = File.join(@later_dir, "#{(@today).strftime('%Y%m%d')}.mytask.3d.txt")
    assert File.exist?(expected_daily_file), "Daily repeating task was not created."

    # Verify strict daily repetition
    expected_strict_daily_file = File.join(@later_dir, "#{(@today + 8).strftime('%Y%m%d')}.mytask.@8d.txt")
  ...
  def test_weekly_repeating_task
    ...
    expected_weekly_file = File.join(@later_dir, "#{(@today).strftime('%Y%m%d')}.mytask.2w.txt")
    assert File.exist?(expected_weekly_file), "Weekly repeating task was not created."

    # Verify strict weekly repetition
    expected_strict_weekly_file = File.join(@later_dir, "#{(@today + 35).strftime('%Y%m%d')}.mytask.@5w.txt")
  ...
  def test_monthly_repeating_task
    ...
    expected_monthly_file = File.join(@later_dir, "#{(@today << 3).strftime('%Y%m%d')}.mytask.3m.txt")
    assert File.exist?(expected_monthly_file), "Monthly repeating task was not created."

    # Verify strict monthly repetition
    expected_strict_monthly_file = File.join(@later_dir, "#{(@today << 6).strftime('%Y%m%d')}.mytask.@6m.txt")
