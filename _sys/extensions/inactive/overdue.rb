...
# Removes ⚠️ emoji from tasks that are not overdue.
...
def process_directory(directory)
  Dir.foreach(directory) do |filename|
    ...
    if filename =~ /^(\d{4}-\d{2}-\d{2})\.(.+)(\..+)$/
      ...
      # Remove ⚠️ emoji from non-overdue tasks
      if due_date >= Date.today && task_name.start_with?('⚠️')
        new_task_name = task_name.sub(/^⚠️/, '') # Remove the ⚠️ emoji
        ...
        begin
          ...
          puts "Removed ⚠️ from #{filename} -> #{new_filename}" # Debugging output
        rescue => e
