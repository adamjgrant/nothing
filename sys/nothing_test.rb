...
class TestDueDateMovement < Minitest::Test
  def test_due_date_movement
    ...
    refute File.exist?(File.join(LATER_DIR, "#{(Date.today - 1).strftime('%Y%m%d')}.Task overdue.txt")),
           "Overdue task should no longer be in 'later'."
    refute File.exist?(File.join(LATER_DIR, "#{Date.today.strftime('%Y%m%d')}.Task due today.txt")),
           "Due-today task should no longer be in 'later'."

    # Verify future-dated and no-date tasks remain in 'later/'
    assert File.exist?(File.join(LATER_DIR, "#{(Date.today + 1).strftime('%Y%m%d')}.Task due tomorrow.txt")),
